...
{
    if (_title == nil) {
        self.title = [[UILabel alloc] initWithFrame:CGRectMake(10, 10, kWidth-20, 20)];
...
{
    if (_backImage == nil) {
        self.backImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.descriptionLabel.frame), kWidth, CGRectGetWidth(self.title.frame)/2)];
...
- (void)setVideo:(VideoListModel *)video
{
    self.title.text = video.title;
    self.descriptionLabel.text = video.descriptionStr;
